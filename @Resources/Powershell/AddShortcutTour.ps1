...
function Check {
    If (Test-Path -Path "$Startpath\Microsoft\Windows\Start Menu\Programs\JaxCore.lnk") {
        $RmAPI.Bang('[!SetOption Card1.Button2.Shape MeterStyle "CardSquareButton:S | Card1.ButtonProg.True"]')
        $RmAPI.Bang('[!UpdateMeter Card1.Button2.Shape]')
        $RmAPI.Bang('[!Redraw]')
    ...
    If (Test-Path -Path "$DesktopPath\JaxCore.lnk") {
        $RmAPI.Bang('[!SetOption Card1.Button1.Shape MeterStyle "CardSquareButton:S | Card1.ButtonDesk.True"]')
        $RmAPI.Bang('[!UpdateMeter Card1.Button1.Shape]')
        $RmAPI.Bang('[!Redraw]')
